...
class TestClassificator(unittest.TestCase):
    ...
    def test_predictions(self):
        ...
        This function iterates over the variables in the `texts` module and checks if each
        variable starts with an underscore. If it does not, it predicts the maximum probability
        for the variable using the `predict_max` method of the `classificator` object. The
        predicted probability is then compared with 0.2 using the `assertGreaterEqual` method of
